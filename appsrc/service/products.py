...
def set_default_product_schema(domain_id):
    # service
    ps = ProductSchema.query.filter_by(domain_id=domain_id).first()
    if ps:
        return ps
    try:
        ps = ProductSchema(domain_id=domain_id)
        db.session.add(ps)
        db.session.flush()
    except Exception as e:
        db.session.rollback()
        raise err.FormatError('Could not create product schema')
    return ps
...
def get_products_filtered_by_group(domain_id, groups=None):
    ...
    try:
        for g in groups:
            subq = (ProductGroupOption
                    .filter(ProductGroupOption.domain_id==domain_id,
                            ProductGroupOption.group_id.in_(g['options']),)
                    .subquery())
        ...
        raise err.FormatError('Missing "options" key for group')
    except:
        raise err.FormatError('Could not collect products')
...
def get_product(product_id, domain_id, visible=None):
    # service
    notfound = lambda: err.NotFound('Product not found')
    if not product_id:
        raise notfound()
    product_id = clean_uuid(product_id)
    q = Product.query.filter_by(product_id=product_id, domain_id=domain_id)
    if visible is not None:
        q = q.filter(Product.visible==visible)
    try:
        return q.one()
