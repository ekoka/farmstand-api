...
def get_public_root(domain, lang):
    ...
    rv._l(f'{app.config.API_NAMESPACE}:public_domain', api_url(
        'api.get_public_domain', domain_name="{domain}"), unquote=True, templated=True)
    rv._l(f'{app.config.API_NAMESPACE}:public_products', api_url(
        'api.get_public_products', domain=domain.name))
    rv._l(f'{app.config.API_NAMESPACE}:public_groups', api_url(
        'api.get_public_groups', domain=domain.name))
    rv._l(f'{app.config.API_NAMESPACE}:public_product_schema', api_url(
        'api.get_public_product_schema', domain=domain.name))
    rv._l(f'{app.config.API_NAMESPACE}:public_product_resources', api_url(
        'api.get_public_product_resources', domain=domain.name))
    rv._l(f'{app.config.API_NAMESPACE}:public_inquiries', api_url('api.post_public_inquiry'))
    rv._l(f'{app.config.API_NAMESPACE}:public_product', api_url(
