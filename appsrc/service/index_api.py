...
def get_root():
    ...
    rv._l(f'{app.config.API_NAMESPACE}:domain',
          api_url('api.get_domain', domain_name='{domain}'),
        templated=True, unquote=True)
    rv._l(f'{app.config.API_NAMESPACE}:domain_name_check', api_url(
        'api.get_domain_name_check', name='{domain}'), templated=True, unquote=True)
    rv._l(f'{app.config.API_NAMESPACE}:public_root', api_url( 'api.get_public_root', domain='{domain}'), templated=True, unquote=True)
    ...
    rv._l(f'{app.config.API_NAMESPACE}:access_request', api_url(
        'api.get_access_request', domain_id='{domain_id}'), templated=True, unquote=True)
